...
const safeRedisCall = async (method, ...args) => {
    try {
        if (method === 'setEx') {
            // Handle setEx: key, seconds, value
            const [key, seconds, value] = args;
            return await redis.setex(key, seconds, value);
        }
        
        if (method === 'keys') {
            // Your redis.js doesn't implement keys properly, return empty array
            return [];
        }
        
        return await redis[method](...args);
    } catch (error) {
        log.error(`Redis ${method} error:`, error);
        return null;
    }
};
...
// FIXED: Simple skip-to-page conversion
const determinePageFromSkip = (skip) => {
    // Convert skip (0, 20, 40, 60...) to page (1, 2, 3, 4...)
    // TMDB pages are 1-indexed and each page has 20 results
    const page = Math.floor(skip / 20) + 1;
    log.debug(`Skip ${skip} converted to page ${page}`);
    return page;
};
...
const fetchData = async (endpoint, params = {}, tmdbApiKey = null, providerId = null, ageRange = null, rating = null, genre = null, year = null, language = 'en') => {
    ...
    // FIXED: Use simple page calculation
    const page = determinePageFromSkip(skip || 0);
